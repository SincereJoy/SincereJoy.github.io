...
C++的高效编程视情况而变，取决于使用的是C++的哪一部分。举例来说，对于传值和传引用的选择，如果使用的是C++中C语言的成分，传值比传引用高效；对于面向对象编程的部分，传常引用更高效；使用模板的泛型编程也是传常饮用更高效；而对于STL部分，迭代器和函数对象都是基于C指针实现的，所以对于STL迭代器和函数对象而言，传值更高效。条款20详细地介绍了传参方式地选择。
